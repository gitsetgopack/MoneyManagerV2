...
def post_date_input(message, bot, date_entered, amount_value, selected_category):
    try:
        ...
        msg = bot.send_message(
            chat_id,
            "Do you want to upload a receipt image (if available)?",
            reply_markup=markup,
        )
        bot.register_next_step_handler(
            msg,
            handle_receipt_decision,
            bot,
            amount_str,
            category_str,
            date_str,
...
def handle_uploaded_receipt(message, bot):
    try:
        ...
        receipt_file_name = os.path.join(
            "receipts",
            f"receipt_{chat_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{file_extension}",
        )
...
def handle_receipt_decision(
    message, bot, amount_str, category_str, date_str, selected_category
):
    try:
        chat_id = message.chat.id
        ...
        if decision == "yes, upload receipt":
            ...
            bot.send_message(
                chat_id,
                'Invalid choice. Please select "Yes, upload receipt" or "No, I\'m done".',
            )
