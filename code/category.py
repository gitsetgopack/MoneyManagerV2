...
# User have three functionality can choose, add a category, delete a category or view the current categories
    ...
    try:
        ...
        if op == options["add"]:
            ...
            for c in helper.getCategories(selectedType):
                markup.add(c)
            # Handle the exception to delete the last category
...
def category_delete(message, bot, selectedType):
    ...
    # Handle the exception to deleting a not exist category
    if find_to_delete == False:
        bot.send_message(
            chat_id,
            "Cannot find the category QAQ",
        ...
        f.close()
        bot.send_message(
            chat_id, 'Delete category "{}" successfully!'.format(category_name)
        )
