...
def process_date_input(message):
    try:
        ...
        try:
            ...
            bot.send_message(
                chat_id,
                "Invalid date format. Please use YYYY-MM-DD or YYYYMMDD format.",
            )
            return

        # Search for files in the receipts folder with the dates in their names
        matching_receipts = glob.glob(f"receipts/*{user_date}*")
        ...
        if matching_receipts:
            try:
                ...
                logging.exception("An error occurred while sending images: " + str(e))
        else:
            bot.send_message(chat_id, "No receipts for this date.")
