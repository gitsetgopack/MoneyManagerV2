...
class TestReceiptProcessor(unittest.TestCase):
    ...
    def test_run_function(self, mock_telebot):
        ...
        mock_bot.send_message.assert_called_once_with(
            self.chat_id, "Please send me a photo of your receipt."
        )
    ...
    def test_handle_receipt_image_success(self, mock_process_receipt):
        ...
        mock_result = {"date": "2024-03-21", "amount": 100.50, "category": "Groceries"}
    ...
    def test_handle_receipt_image_no_photo(self):
        ...
        self.mock_bot.send_message.assert_called_once_with(
            self.chat_id, "Please send a valid receipt photo."
        )
    ...
    )  # Mocking the send_message to avoid actual API calls
    def test_add_user_record_new_user(
        self, mock_send_message, mock_write_json, mock_read_json
    ):
        """Test adding a record for a new user"""
        # Arrange
        mock_read_json.return_value = {}
        record = {"date": "2024-03-21", "amount": 100.50, "category": "Groceries"}
    ...
    def test_add_user_record_existing_user(
        self, mock_send_message, mock_write_json, mock_read_json
    ):
