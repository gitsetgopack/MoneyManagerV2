...
@authenticate
async def categories_delete(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> int:
...
@authenticate
async def confirm_delete_category(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> int:
    ...
    if query.data.startswith("delete_"):
        ...
        if response.status_code == 200:
            await query.message.edit_text("✅ Category deleted successfully!\nClick /categories_view to see the updated list.")
        else:
            error_detail = response.json().get("detail", "Unknown error")
            await query.message.edit_text(f"❌ Failed to delete category: {error_detail}")
