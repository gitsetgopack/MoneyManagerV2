...
async def accounts_view(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> None:
    ...
    if response.status_code == 200:
        ...
        items_per_page = 5
        total_pages = len(sorted_accounts) // items_per_page + (1 if len(sorted_accounts) % items_per_page else 0)
        paginator = InlineKeyboardPaginator(
            total_pages,
            current_page=page,
            data_pattern="view_accounts#{page}"
        )
...
@authenticate
async def accounts_view_page(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> None:
...
@authenticate
async def accounts_add(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> int:
    """Start the account addition process."""
    await update.message.reply_text("Please enter the account name:")
    return ACCOUNT_NAME

async def handle_account_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle the account name input."""
    account_name = update.message.text
    context.user_data["account_name"] = account_name
    await update.message.reply_text("Please enter the initial balance for this account:")
...
@authenticate
async def confirm_delete_account(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> int:
    ...
    if query.data.startswith("delete_"):
        ...
        if response.status_code == 200:
            await query.message.edit_text("✅ Account deleted successfully!\nClick /accounts_view to see the updated list.")
        else:
            error_detail = response.json().get("detail", "Unknown error")
...
@authenticate
async def accounts_update(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str) -> int:
